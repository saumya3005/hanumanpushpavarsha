...
export default function MembersPage() {
  ...
  const filteredMembers = allMembers.filter((member) => {
    const memberName = language === "hi" ? member.name.hi : member.name.en;
    ...
    const matchesRole =
      selectedRole === "members.role.all" || member.roleKey === selectedRole;
            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filteredMembers.map((member, index) => (
                <MemberCard
                  ...
                  member={{
                    ...member,
                    name: language === "hi" ? member.name.hi : member.name.en,
                    role: member.customRole || getTranslated(member.roleKey),
                    description:
